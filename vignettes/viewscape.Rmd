...
### 1.2 Compute visual magnitude

```{r}
vm <- viewscape::visual_magnitude(test_dsm, output)
terra::plot(vm, axes=FALSE, box=FALSE, legend = FALSE)
```

### 1.3 Subset viewshed using by specifying the field of view

```{r}
sector <- viewscape::sector_mask(output, c(40,160))
```
...
viewpoint[1,1] <- test_viewpoint[1]
viewpoint[1,2] <- test_viewpoint[2]
output_r <- viewscape::visualize_viewshed(sector, outputtype = 'raster')
terra::plot(test_dsm, axes=FALSE, box=FALSE, legend = FALSE)
terra::plot(output_r, add=TRUE, col = "red", axes=FALSE, box=FALSE, legend = FALSE)
terra::plot(sp::SpatialPoints(viewpoint), add = TRUE, col = "blue", axes=FALSE, box=FALSE, legend = FALSE)
```
