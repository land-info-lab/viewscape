...
#Compute viewshed
output <- viewscape::compute_viewshed(dsm = test_dsm, viewpoints = test_viewpoint, offset_viewpoint = 6, plot=TRUE)
```
...
The land use/cover diversity analysis calculates the percentage(%) of each type of land use/ cover within a viewshed to get the Shannon Diversity Index. 

```{r eval=FALSE}
# load landuse raster
test_landuse <- raster::raster(system.file("test_landcover.tif",
                                           package ="viewscape"))
raster::plot(test_landuse, 
             breaks = c(1,2,3,4), 
...
test_diversity <- calculate_diversity()
```

# 2.3 calculate a single feature

The analysis is to calculate the percentages(%) of a feature (including trees, buildings, parking, and roads) within the viewshed.
