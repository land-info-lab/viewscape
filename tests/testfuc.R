...
# rescope the DSM
x_min <- raster::extent(dsm)[1] + x_range/2.1
x_max <- raster::extent(dsm)[2] - x_range/2.1
y_min <- raster::extent(dsm)[3] + y_range/2.1
y_max <- raster::extent(dsm)[4] - y_range/2.1
# crop DSM
...
viewpoint[1,1] <- col #test_viewpoint[1]
viewpoint[1,2] <- row #test_viewpoint[2]
viewpoint[1,3] <- z_viewpoint
colnames(viewpoint) <- c('x', 'y', 'z')

# convert raster to points
#sample_points <- raster::rasterToPoints(s_dsm)
dsm_matrix <- raster::as.matrix(s_dsm)
n_cell <- raster::ncell(s_dsm)
output <- visibleLabel(viewpoint, dsm_matrix, n_cell, round(raster::res(s_dsm)[1]))
raster_data <- raster::raster(output)
raster::extent(raster_data) <- raster::extent(s_dsm)
raster::res(raster_data) <- raster::res(s_dsm)
