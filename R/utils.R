...
samplealongline <- function(dsm, modified_dsm=NULL, dem=NULL, viewpoint, h1, h2, samplecoordinates){
  ...
  # h2 is the height of sample coordinates. the unit of input should be meter
  ...
  z_viewpoint = dsm[raster::cellFromXY(dsm,cbind(viewpoint[1],viewpoint[2]))] + h1 #the height of viewpoints based on dsm
  ...
  if(is.null(modified_dsm) == FALSE){# if input modified_dsm is detected, compare the height of viewpoint on dsm to the one on dem
    ...
    z_viewpoint2 = dem[raster::cellFromXY(dem,cbind(viewpoint[1],viewpoint[2]))] + h1 #the height of viewpoints based on dem
  ...
  z_sample = dsm[raster::cellFromXY(dsm,cbind(samplecoordinates[1],samplecoordinates[2]))] + h2 #set elevation of sample
...
visiblesample <- function(dsm, modified_dsm, dem, viewpoint, h1, h2, samplecoordinates){
  ...
  # h2 is the height of sample coordinates. the unit of input should be meter
  # samplecoordinates is the coordinates of a certain cell(point) of a DSM
  xyzzl <- samplealongline(dsm, modified_dsm = modified_dsm, dem = dem, viewpoint, h1, h2, samplecoordinates)
...
calculate_viewshed <- function(dsm, under=NULL, dem=NULL, viewpoint, h1=1.7, h2=0, sample_points=NULL, r = NULL){
  ...
  for(i in 1:nrow(sample_points)){
    ...
    visibles <- visiblesample(dsm, modified_dsm = under, dem = dem, viewpoint, h1, h2, samplecoordinates) #check visibility
