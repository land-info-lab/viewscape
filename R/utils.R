...
samplealongline <- function(dsm, modified_dsm=NULL, dem=NULL, viewpoint, h, samplecoordinates){
  ...
  z_viewpoint = dsm[raster::cellFromXY(dsm,cbind(viewpoint[1],viewpoint[2]))] + h #the height of viewpoints based on dsm
  ...
  if(is.null(modified_dsm) == FALSE){# if input modified_dsm is detected, compare the height of viewpoint on dsm to the one on dem
    ...
    z_viewpoint2 = dem[raster::cellFromXY(dem,cbind(viewpoint[1],viewpoint[2]))] + h #the height of viewpoints based on dem
...
visiblesample <- function(dsm, modified_dsm, dem, viewpoint, h, samplecoordinates){
  ...
  # dem is the raster of terrain without any vertical object. If input of dsm is already digital elevation model, this arguement can be ignored
  # viewpoint is a matrix including x,y coordinates
  # h is the height of viewpoint
  # samplecoordinates is the coordinates of a certain cell(point) of a DSM
  xyzzl <- samplealongline(dsm, modified_dsm = modified_dsm, dem = dem, viewpoint, h, samplecoordinates)
...
calculate_viewshed <- function(dsm, under=NULL, dem=NULL, viewpoint, h, sample_points=NULL, r = NULL){
  ...
  for(i in 1:nrow(sample_points)){
    ...
    visibles <- visiblesample(dsm, modified_dsm = under, dem = dem, viewpoint, h, samplecoordinates) #check visibility
